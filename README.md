...
- **Offer pages (TSL/VSL):** both affiliate links live in `assets/config.js`.
  - `VSL` = link **1** (`uid=566`) — used as the **primary** CTA target (VSL converts best behind a strong landing page, which this is).
  - `TSL` = link **2** (no `uid`).
  - Flip the primary in one edit: `window.NV.primary = "TSL"`.
  - ⚠️ The raw links you sent used malformed macros (`{gbraid|`). Normalized to `{gclid}` / `{gbraid}` / `{wbraid}` so substitution works.
